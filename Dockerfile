...
ARG GDPR_URL=https://github.com/GregThib/ttrss-tumblr-gdpr/archive/master.tar.gz
...
RUN wget -q -O- ${TTRSS_URL} | tar -xzC . --strip-components 1 && \
    ...
    wget -q -O- ${GDPR_URL} | tar -xzC plugins/ --strip-components 1 --one-top-level=tumblr-gdpr && \
